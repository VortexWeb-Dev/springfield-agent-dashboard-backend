...
class PerformanceController extends BitrixController
{
    ...
    private function getMonthlyPerformanceData(string $id, int $month, int $year, string $userEmail): array
    {
        ...
        $ads = $this->getAllUserAds(['ufCrm95AgentEmail' => $userEmail], [
            'ufCrm95Status',
            'ufCrm95PfEnable',
            'ufCrm95BayutEnable',
            'ufCrm95DubizzleEnable',
            'ufCrm95WebsiteEnable',
            'ufCrm95Price',
            'createdTime'
        ]);
        ...
        $deals = $this->getDeals([
            ...
            '>=CLOSEDATE' => $startDate,
            '<=CLOSEDATE' => $endDate
        ], ['ID', 'CLOSEDATE', 'OPPORTUNITY', 'STAGE_ID', 'UF_CRM_1727871937052', 'CLOSED']) ?? [];
        ...
        // Ad categorization
        $published = array_filter($ads, fn($ad) => $ad['ufCrm95Status'] === 'PUBLISHED');
        $live = array_filter($ads, fn($ad) => $ad['ufCrm95Status'] === 'LIVE');
        $draft = array_filter($ads, fn($ad) => $ad['ufCrm95Status'] === 'DRAFT');

        $pf = array_filter($published, fn($ad) => $ad['ufCrm95PfEnable'] === 'Y');
        $bayut = array_filter($published, fn($ad) => $ad['ufCrm95BayutEnable'] === 'Y');
        $dubizzle = array_filter($published, fn($ad) => $ad['ufCrm95DubizzleEnable'] === 'Y');
        $website = array_filter($published, fn($ad) => $ad['ufCrm95WebsiteEnable'] === 'Y');

        $worth = array_sum(array_map(
            fn($ad) => (float)$ad['ufCrm95Price'],
            $published
        ));
        ...
        $grossCommission = array_sum(array_map(fn($deal) => (float)$deal['UF_CRM_1727871937052'], $closedDeals));
